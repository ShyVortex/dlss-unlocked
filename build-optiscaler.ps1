...
if ($CreateStandaloneZip) {
    ...
    if (Test-Path "$DllVersionDir\OptiScaler.ini") {
        ...
        if ($optiIniContent -match '(?m)^UseHQFont\s*=') {
            $optiIniContent = $optiIniContent -replace '(?m)^UseHQFont\s*=.*', 'UseHQFont=false'
        } else {
            $optiIniContent = $optiIniContent -replace '\[Menu\]', "[Menu]`r`nUseHQFont=false"
        }
        if ($optiIniContent -match '(?m)^HUDFixDontUseSwapchainBuffers\s*=') {
            $optiIniContent = $optiIniContent -replace '(?m)^HUDFixDontUseSwapchainBuffers\s*=.*', 'HUDFixDontUseSwapchainBuffers=true'
        } else {
            $optiIniContent = $optiIniContent -replace '\[FrameGen\]', "[FrameGen]`r`nHUDFixDontUseSwapchainBuffers=true"
        }
        if ($optiIniContent -match '(?m)^UseMutexForSwapchain\s*=') {
            $optiIniContent = $optiIniContent -replace '(?m)^UseMutexForSwapchain\s*=.*', 'UseMutexForSwapchain=false'
        } else {
            $optiIniContent = $optiIniContent -replace '\[FrameGen\]', "[FrameGen]`r`nUseMutexForSwapchain=false"
        }
        ...
        Write-Host "  Configured OptiScaler.ini (FGInput=nvngxfg, FGOutput=auto, FGNvngxReplacement=Arturs, ManualInputPolling=true, UseHQFont=false, HUDFixDontUseSwapchainBuffers=true, UseMutexForSwapchain=false)" -ForegroundColor Gray
    }
    if (Test-Path "$DllVersionDir\nvngx.dll_dlssnr.dll") {
